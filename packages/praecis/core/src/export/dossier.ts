...
import { renderDeepLink, renderLabel } from './deep-links.js';
import { getStringMetadata, getNumberMetadata, normalizeText, truncateText, toNumber, uniqueSortedStrings } from '../utils/graph-helpers.js';
...
export class DossierExporter {
  ...
  async buildVideoDossier(videoId: string, options: DossierBuildOptions = {}): Promise<Result<VideoDossier>> {
    ...
    for (const claim of claimsResult.value.items) {
      ...
      const locator = locatorRaw && typeof locatorRaw === 'object' && 'kind' in (locatorRaw as object)
        ? locatorRaw as Locator
        : undefined;
