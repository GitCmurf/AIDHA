...
describe('cross-vector dedup integration', () => {
  it('merges RSS and Readwise when both resolve to the same web canonical id', async () => {
    ...
    const rss = await runtime.runVector(rssVector, { ref: 'https://blog.example.com/feed.xml', metadata: { itemGuid: 'item-1' } });
    const readwise = await runtime.runVector(readwiseVector, { ref: 'readwise:book:7' });
  ...
  it('links but does not merge Readwise books without source_url', async () => {
    ...
    const result = await runtime.runVector(readwiseVector, { ref: 'readwise:book:8' });
