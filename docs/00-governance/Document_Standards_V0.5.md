...
|---------|------------|---------------|----------------------------------------------|
| 0.5     | 2025-11-15 | CMF           | Added linkcheck + template guidance          |
...
- **Validation:** `docs/_scripts/gen_catalog.py` + `docs/_scripts/validate_docs.py` run in pre-commit/CI. `docs/_scripts/linkcheck.py` generates `docs/01-indices/linkcheck-report.json`; failures block CI unless skipped via env.
- **RAG Readiness:** Keep headings predictable; avoid inline HTML except for metadata block.
- **Templates:** Copy files from `docs/_templates/` (PRD/ADR/FDD) when creating new documents.
...
`docs/01-indices/package-docs.json` summarizes package README metadata for agents; keep package docs
in sync with canonical entries in `docs/`.
