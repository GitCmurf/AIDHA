...
- 2026-05-10: Extracted harness-only matrix execution for shortlist/refine extraction to
  ...
  down to 1,374 lines.
- 2026-05-10: Stabilized full-suite timing by setting a 30s YouTube package Vitest default and
  explicit budgets for slow integration tests; `pnpm --dir packages/praecis/youtube test` passed
  with 90 test files passed, 1 skipped; 891 tests passed, 6 skipped.
- 2026-05-10: Extracted Google embedding runtime configuration and default-model ownership to
...
- 2026-05-11: Removed the unreachable `buildVideoReports` judge branch whose flag was only ever
  passed `false`; `judgeVideoReports` remains the canonical judge stage, and
  `narrow-manual-baseline.ts` is down to 870 lines.
- 2026-05-11: Extracted video report construction, per-video coverage logging, teacher enrichment,
...
- 2026-05-11: Extracted score-stage resume handling, per-video cache signatures, embedding
  ...
  594 lines.
- 2026-05-11: Extracted shortlist-stage extraction, fallback probing, adaptive prompt-pack
  escalation, shortlist target selection, and shortlist-artifact writes to
  `packages/praecis/youtube/src/eval/narrow-shortlist-stage.ts`; `narrow-manual-baseline.ts` is down
  to 470 lines.
...
- 2026-05-11: Moved mode resolution, signature construction, transcript/baseline loading,
...
- [x] Every AIDHA-TASK-007 item from TD-001 through TD-022 is resolved, superseded with evidence,
