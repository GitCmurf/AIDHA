...
| ------- | ---------- | ------ | ----------------------------- | --------- | ------ | --------- |
| 0.1     | 2025-11-09 | TBD    | Skeleton PRD                  | —         | Draft  | —         |
| 0.2     | 2025-12-27 | CMF    | Meminit DocOps 2.0 migration  | —         | Draft  | —         |
| 0.3     | 2026-01-24 | Codex  | Requirements + contracts      | —         | Draft  | —         |
| 0.4     | 2026-01-28 | CMF    | Tasks graph (future)          | —         | Draft  | —         |
| 0.5     | 2026-01-29 | AI     | Upserts + SQLite backend      | —         | Draft  | —         |
| 0.6     | 2026-02-05 | AI     | Scope + pagination updates    | —         | Draft  | —         |
| 0.7     | 2026-02-05 | AI     | Wrap long lines               | —         | Draft  | —         |
| 0.8     | 2026-02-05 | AI     | Restore version table headers | —         | Draft  | —         |
...
- Cursors MUST be opaque strings generated by the store (clients MUST NOT infer structure).
- Cursors are only valid for the exact sort definition used to produce them (field + direction).
- Invalid or stale cursors MUST be treated as “no cursor” (start of the result set).
- Invalid or stale cursors MUST NOT throw.
- Stores MAY invalidate cursors after data mutations, but this behavior must be documented.
...
- When `cascade` is omitted or false, dangling edges MUST remain.
- Consumers can filter orphaned edges during queries if needed.
...
- For nodes, “matches” means deep equality of `type`, `label`, `content`, and `metadata` only.
- Exclude `createdAt` and `updatedAt` from node comparisons.
- For edges, “matches” means deep equality of `subject`, `predicate`, `object`, and `metadata` only.
- Exclude timestamps from edge comparisons.
- Deep equality comparisons MUST treat object key order as irrelevant.
...
Canonical schemas are defined in `packages/reconditum/src/schema/node.ts` and
`packages/reconditum/src/schema/edge.ts` and exported from the package root as
`GraphNode`, `GraphEdge`, `NodeType`, and `Predicate`. Node-type-specific metadata
expectations are defined by domain PRDs (AIDHA-PRD-002, AIDHA-PRD-003) and ingestion
package docs; this PRD treats them as metadata contracts rather than distinct node schemas.
...
### Optional Implementations (Non-MVP)

- `LevelGraphStore` may be retained for experimentation but is not required for MVP.
- Optional stores must still pass the contract suite if included.
...
## Tasks Graph (Next, Non-MVP)

These are future considerations and are not part of the MVP functional requirements.

- Edge type constraints: `taskDependsOn` edges should connect Task -> Task only.
- Cycle detection: consider rejecting cycles (or marking a plan invalid).
- Scheduling semantics: edge metadata may include dependency type + lag; tasks may include
  duration/effort/earliest start constraints.
- Versioning: snapshots or “plans” as first-class artifacts so schedule reasoning is stable.
